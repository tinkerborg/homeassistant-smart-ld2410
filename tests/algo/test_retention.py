...
from custom_components.smart_ld2410.algo.retention import OwnershipRetention
...
def _settle(tracker: OwnershipRetention, ts: float, seconds: float) -> float:
...
def _sit_down(tracker: OwnershipRetention, ts: float, seconds: float) -> float:
...
def test_a_rise_in_the_still_channel_reads_positive() -> None:
    ...
    tracker = OwnershipRetention(_config())
...
def test_a_departure_never_reads_as_presence() -> None:
    ...
    tracker = OwnershipRetention(_config(tau_peak=0.0))
...
def test_the_peak_is_held_across_a_quiet_stretch() -> None:
    ...
    tracker = OwnershipRetention(_config(tau_peak=90.0))
...
def test_retention_only_answers_for_the_occupancys_own_gates() -> None:
    ...
    config = _config()
    tracker = OwnershipRetention(config)
    ...
    assert not tracker.refreshes_hold(ts, scored=False)
...
def test_retention_holds_a_disarmed_room_until_it_decays() -> None:
    ...
    ts = _sit_down(tracker, ts, 10.0)
    assert tracker.refreshes_hold(ts, scored=False)
...
def _walk(
    tracker: OwnershipRetention, ts: float, frames: int, gate: int | None
) -> float:
    """Feed ``frames`` frames of activity leading at ``gate``."""
    for _ in range(frames):
        tracker.observe_lead(ts, gate)
        tracker.refreshes_hold(ts, scored=True)
        ts += FRAME_S
    return ts
...
def test_the_entry_walk_in_cannot_arm_release() -> None:
    ...
    _walk(tracker, 0.0, config.cross_n * 3, NEAR_GATE)
...
def test_re_crossing_the_entry_band_arms_release() -> None:
    ...
    ts = _walk(tracker, 0.0, config.cross_n * 3, NEAR_GATE)
    ts = _walk(tracker, ts + config.quiet_s, 1, None)
    _walk(tracker, ts, config.cross_n, NEAR_GATE)

    assert tracker.state == OWNERSHIP_ARMED


def _arm(tracker: OwnershipRetention, config: DetectorConfig) -> float:
    """Drive a fresh ownership through to armed, returning the frame time."""
    tracker.enter((NEAR_GATE,))
    ts = _walk(tracker, 0.0, config.cross_n, NEAR_GATE)
    ts = _walk(tracker, ts + config.quiet_s, 1, None)
    ts = _walk(tracker, ts, config.cross_n, NEAR_GATE)
...
def test_an_armed_room_is_held_only_by_attributed_evidence() -> None:
    ...
    assert tracker.refreshes_hold(ts, scored=True)

    ts += config.grace_s + 1.0
    tracker.observe_lead(ts, FAR_GATE)

    assert not tracker.refreshes_hold(ts, scored=True)
...
def test_state_round_trip_preserves_retention_and_ownership() -> None:
    ...
    restored = Detector.retention_from_state(payload, config)
    assert restored is not None

    assert restored.values == detector.retention.values
    assert restored.state == detector.retention.state == OWNERSHIP_DISARMED

    for frame in stream.burst(5.0, still={SEAT_GATE: 40}):
        detector.retention.update(frame.ts_mono, frame.still_gates)
...
def test_state_from_before_this_phase_starts_the_statistic_fresh() -> None:
    ...
    assert Detector.retention_from_state(payload, detector.config) is None
